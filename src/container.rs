...
fn docker_ps(_verbose: u8) -> Result<()> {
    ...
    if stdout.trim().is_empty() {
        ...
        println!("{}", rtk);
        tracking::track("docker ps", "rtk docker ps", &raw, &rtk);
        return Ok(());
    }
    ...
    if count > 15 { rtk.push_str(&format!("  ... +{} more", count - 15)); }
...
fn docker_images(_verbose: u8) -> Result<()> {
    ...
    if lines.is_empty() {
        ...
        println!("{}", rtk);
        tracking::track("docker images", "rtk docker images", &raw, &rtk);
        return Ok(());
    }
    ...
    if lines.len() > 15 { rtk.push_str(&format!("  ... +{} more", lines.len() - 15)); }
...
fn docker_logs(args: &[String], _verbose: u8) -> Result<()> {
    ...
    println!("{}", rtk);
    tracking::track(&format!("docker logs {}", container), "rtk docker logs", &raw, &rtk);
    Ok(())
}
...
fn kubectl_pods(args: &[String], _verbose: u8) -> Result<()> {
    ...
    let json: serde_json::Value = match serde_json::from_str(&raw) {
        ...
        Err(_) => {
            ...
            println!("{}", rtk);
            tracking::track("kubectl get pods", "rtk kubectl pods", &raw, &rtk);
            return Ok(());
        }
    };
    ...
    if !issues.is_empty() {
        ...
        if issues.len() > 10 { rtk.push_str(&format!("  ... +{} more", issues.len() - 10)); }
    }
...
fn kubectl_services(args: &[String], _verbose: u8) -> Result<()> {
    ...
    let json: serde_json::Value = match serde_json::from_str(&raw) {
        ...
        Err(_) => {
            ...
            println!("{}", rtk);
            tracking::track("kubectl get svc", "rtk kubectl svc", &raw, &rtk);
            return Ok(());
        }
    };
    ...
    if services.len() > 15 { rtk.push_str(&format!("  ... +{} more", services.len() - 15)); }
...
fn kubectl_logs(args: &[String], _verbose: u8) -> Result<()> {
    ...
    println!("{}", rtk);
    tracking::track(&format!("kubectl logs {}", pod), "rtk kubectl logs", &raw, &rtk);
    Ok(())
}
